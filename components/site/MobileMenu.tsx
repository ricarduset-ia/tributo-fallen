    ...
    <div className="md:hidden">
      ...
      <div
        ...
        className={cn(
          ...
          open ? "opacity-85 pointer-events-auto" : "opacity-0 pointer-events-none"
        )}
      />

      {/* Drawer */}
      <div className={cn(
        "fixed top-0 right-0 z-50 h-full w-72 bg-fallen-void border-l border-fallen-bone/10 flex flex-col transition-transform duration-300 ease-out",
        ...
        <nav className="flex flex-col px-6 py-8 gap-0 flex-1">
          {links.map((l) => (
            <Link
              ...
              href={l.href}
              onClick={() => setOpen(false)}
              className="group flex items-center justify-between py-4 border-b border-fallen-bone/8 text-mono text-xs uppercase tracking-widest text-fallen-muted hover:text-fallen-gold transition-colors"
            >
              {l.label}
              <span className="text-fallen-bone/20 group-hover:text-fallen-gold group-hover:translate-x-0.5 transition-all">→</span>
            </Link>
          ))}
        </nav>
